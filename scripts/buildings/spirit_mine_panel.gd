...
const BID = "spirit_mine"
const NAME = "灵矿场"
const COLOR = Color(0.55, 0.7, 0.95)
const RATE_NAME = "灵矿"
const BASE_COST = 30
...
func refresh():
	...
	iv.add_child(_lbl("开采灵脉，随时间产出" + RATE_NAME, Color(0.7, 0.7, 0.8), 12))
	_ore_label = _lbl("当前库存：" + RATE_NAME + " " + _fmt(_ore), Color(0.9, 0.75, 0.3), 13)
	iv.add_child(_ore_label)
	iv.add_child(_lbl("当前产量：" + _fmt(_ore_rate) + " " + RATE_NAME + "/秒", Color(0.4, 0.9, 0.5), 12))
	...
	var desc = _lbl(NAME + "开采灵脉矿石，" + RATE_NAME + "用于洞府扩建。", COLOR, 12)
...
func tick_upgrade(building: Dictionary, ore: float = 0.0, wood: float = 0.0, ore_rate: float = 0.0, wood_rate: float = 0.0):
	...
	_ore_rate = ore_rate
	if _ore_label:
		_ore_label.text = "当前库存：" + RATE_NAME + " " + _fmt(_ore)
